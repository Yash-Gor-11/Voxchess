...
export function useNavVoice() {
  ...
  const activate = useCallback(() => {
    ...
    if (activeMode === "nav") { stop(); setActive(null); setStatus("idle"); return; }
    setActive("nav"); setStatus("listening"); setTranscript(""); setResult(null);
    handleRef.current = startRecognition({
      onResult: (t, isFinal) => {
        setTranscript(t);
        if (!isFinal) return;
        const cmd = parseNavPhrase(t);
        if (!cmd.ok) { setStatus("error"); setResult({ ok: false, message: cmd.message }); toast.error(cmd.message ?? "Unknown command"); }
        else if (cmd.action === "signout") { supabase.auth.signOut(); setStatus("success"); setResult({ ok: true, message: "Signing out…" }); }
        else if (cmd.to) { navigate({ to: cmd.to as any }); setStatus("success"); setResult({ ok: true, message: `Going to ${cmd.to}` }); }
