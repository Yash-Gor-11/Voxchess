...
export const Route = createFileRoute("/auth/signup")({
  head: () => ({
    meta: [
      { title: "Sign up — VoxChess" },
      { name: "description", content: "Create your VoxChess account." },
    ],
  }),
...
function SignupPage() {
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<Form>({ resolver: zodResolver(schema), defaultValues: { terms: false } });
      ...
      <Card className="w-full max-w-[420px] p-8 bg-card/70 backdrop-blur border-border/50">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-3">
          ...
          <Button
            type="submit"
            disabled={loading}
            className="w-full bg-[var(--accent-blue)] hover:opacity-90 text-white"
          >
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          ...
          <Link to="/auth/login" className="text-[var(--accent-blue)] font-medium">
            Log in
          </Link>
        </p>
      </Card>
